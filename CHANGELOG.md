...
- **Portal CSS minifier + Make targets:** new [`scripts/minify_portal_css.py`](scripts/minify_portal_css.py) (paren-aware, idempotent — preserves `calc(...)` / `var(...)` arithmetic and `/*! ... */` legal comments) plus `make minify-css` and `make minify-css-check` targets in [`Makefile`](Makefile). Applied across every stylesheet under `services/frontend/portal/assets/` (`docs.css` −6 354 lines, `internal-layout.css` −2 953, `home.css` −2 055, `docs-premium.css` −1 165, `public-layout.css` −586, `docs-theme.css` −527, `docs-site-nav.css` −267, `docs-syntax-theme.css` −108, `portal-router.css` −100); ~11 360 lines collapsed to minified single-line form.
...
- **Portal router landing redesign:** [`services/portal/index.html`](services/portal/index.html) restyled around a `portal-router` shell — flashlight theme toggle button, hero `portal-router__title`, and refreshed copy. Old `docs-top-nav` mount and inline `card` wrapper retired in favour of the dedicated layout.
