...
# Extra top-level leaves inside a service dir that aren't part of the SPINE and
# don't sit under a diataxis subdir. Emitted as sidebar children between the
# spine and the diataxis subtrees. Add here (not by hand-editing nav-tree —
# the whole `services` subtree is regenerated by this script and hand-edits
# are lost).
NAV_SERVICE_EXTRAS: dict[str, tuple[tuple[str, str], ...]] = {
    "datastore": (
        ("schema.html", "Schema reference"),
        ("data-lifecycle.html", "Data lifecycle"),
    ),
}

# Extra top-level leaves inside services/ that aren't per-service — appear as
# direct children of the "services" nav node, above the service list.
NAV_SERVICES_HUB_EXTRAS: tuple[tuple[str, str], ...] = (
    ("deployment-topology.html", "Deployment topology"),
)
...
def _build_services_subtree(services: list[dict]) -> list[dict]:
    children: list[dict] = []
    for filename, label in NAV_SERVICES_HUB_EXTRAS:
        if not (SERVICES_DIR / filename).exists():
            continue
        children.append(
            {
                "id": f"services-{_id_safe(Path(filename).stem)}",
                "label": label,
                "href": f"/services/portal/internal/services/{filename}",
            }
        )
    for svc in services:
        name = svc["_name"]
        ...
        for filename, label in NAV_SERVICE_EXTRAS.get(name, ()):
            if not (svc_dir / filename).exists():
                continue
            kids.append(
                {
                    "id": f"services-{name}-{_id_safe(Path(filename).stem)}",
                    "label": label,
                    "href": f"/services/portal/internal/services/{name}/{filename}",
                }
            )
