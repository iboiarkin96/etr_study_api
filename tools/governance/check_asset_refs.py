...
def _is_skippable(ref: str) -> bool:
    ...
    if "…" in ref or "..." in ref:
        return True
    # `services/telegram/storybook-static/` is a build artefact (gitignored,
    # regenerated by `make tma-storybook`) and therefore never on disk in a
    # fresh checkout or CI runner. Ownership of validation is delegated to
    # `tools/docs/check_storybook_refs.py`, which reads the built index and
    # verifies both the story id and the relative-path depth; on a clean
    # checkout it degrades gracefully to path-shape validation. Skipping
    # here keeps `check_asset_refs` green while the story linter owns the
    # class of URL end-to-end.
    if "storybook-static/" in ref:
        return True
    return False
