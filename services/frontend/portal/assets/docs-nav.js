...
// Absolute URL of the assets directory — derived from this script's own src so
// it resolves correctly regardless of how the portal is served (file://, HTTP at
// any root, Docker with a /frontend/portal/assets alias, etc.).
const DOCS_ASSET_DIR = (function () {
...
function activeTarget(relPath) {
  ...
  if (relPath.startsWith("internal/governance/adr/")) {
    return "internal/governance/adr/index.html";
  }
  if (relPath.startsWith("internal/handbook/developer/")) {
    return "internal/handbook/developer/index.html";
  }
  if (relPath.startsWith("internal/governance/backlog/")) {
    return "internal/governance/backlog/index.html";
  }
  if (relPath.startsWith("internal/sre/runbooks/")) {
    return "internal/sre/runbooks/index.html";
  }
  if (relPath.startsWith("internal/governance/rfc/")) {
    return "internal/governance/rfc/index.html";
  }
  ...
  if (relPath.startsWith("internal/handbook/howto/")) {
    return "internal/handbook/howto/index.html";
  }
  if (relPath.startsWith("internal/")) {
    return "internal/index.html";
  }
  if (relPath.startsWith("public/reference/api/")) {
    return "public/reference/api/index.html";
  }
...
const DOCS_NAV_LINK_REGISTRY = {
  ...
  openapi: {
    target: "public/reference/api/index.html",
    icon: "◎",
  ...
  backlog: {
    target: "internal/governance/backlog/index.html",
    icon: "▣",
    navLabel: "Backlog",
    activePrefixes: ["backlog"],
  },
  pdoc: {
    target: "internal/catalog/api/code-reference/index.html",
    icon: "◧",
  ...
  runbooks: {
    target: "internal/sre/runbooks/index.html",
...
function docsQuickLinks(fromDir) {
  ...
  return keys.map((key) => {
    const entry = docsNavEntry(key);
    const rel = String(entry.target || "").replace(/^\/+/, "");
    return {
      label: entry.searchLabel || entry.navLabel || entry.footerLabel || key,
      href: rel ? relHref(fromDir, rel) : "#",
    };
  });
}
...
// Builds the search input scaffolding eagerly so layout is stable, but defers
// loading the search engine (BM25 ranking, suggestions, telemetry, etc.) to
// `docs-search.js` until the first interaction. Saves ~30 KB of parse work on
// every page that doesn't actually use search.
function mountDocsSearch(nav, fromDir) {
  const isPublicAudience = isDocsPublicAudience();
  ...
  function attachIfReady() {
    if (mounted) return;
    const api = window.__DOCS_SEARCH__;
    if (!api || typeof api.mount !== "function") return;
    mounted = true;
    api.mount({ input, results, wrap, fromDir, isPublicAudience, resultsId });
    // Re-fire focus/input so the freshly attached handlers see the user's
    // current intent (preload index, run pending query) without a reload.
  ...
  function ensureSearchBundle() {
    if (mounted) return;
    if (window.__DOCS_SEARCH__) {
      attachIfReady();
      return;
    }
    if (bundleRequested) return;
    bundleRequested = true;
    const existing = document.querySelector('script[data-docs-search-bundle="1"]');
    if (existing) {
      existing.addEventListener("load", attachIfReady);
      return;
    }
    const script = document.createElement("script");
    script.src = DOCS_ASSET_DIR + "docs-search.js";
    script.async = false;
    script.setAttribute("data-docs-search-bundle", "1");
    script.addEventListener("load", attachIfReady);
  ...
  // Trigger lazy load on any first interaction.
  ["focus", "pointerdown", "keydown"].forEach((eventName) => {
    input.addEventListener(eventName, ensureSearchBundle, { once: true, passive: true });
  });

  // Cmd/Ctrl+K opens the search palette across the page; load the bundle then
  // so the input is fully wired by the time the user starts typing.
...
function docsHubHrefForPrefix(prefix) {
  const hubs = {
    ...
    qa: "internal/handbook/qa/index.html",
    rfc: "internal/governance/rfc/index.html",
    runbooks: "internal/sre/runbooks/index.html",
  };
...
function injectDocsPopupsRuntime() {
  ...
  const script = document.createElement("script");
  script.defer = true;
  script.src = DOCS_ASSET_DIR + "docs-popups.js";
...
function ensureInternalLayoutForInternalSections() {
  ...
  if (!document.querySelector('script[src$="assets/internal-sidebar.js"]')) {
    const script = document.createElement("script");
    script.defer = true;
    script.src = DOCS_ASSET_DIR + "internal-sidebar.js";
    document.head.appendChild(script);
  }
}
...
function initAutoInPageToc() {
  ...
  const isInternalReadmeHub = relPath === "internal/index.html";
...
function initDocsSiteFooter() {
  ...
  const INTERNAL_GROUPS = [
    {
      ...
      links: [
        ...
        { href: "internal/index.html", text: "Internal docs" },
    ...
    },
    {
      label: "Guides",
      links: [
        { href: "internal/handbook/howto/0001-onboarding-from-zero-to-endpoint-docs.html", text: "Onboarding" },
        { href: "internal/handbook/howto/index.html", text: "How-to guides" },
        { href: "internal/sre/runbooks/index.html", text: "Runbooks" },
        { href: "internal/handbook/developer/index.html", text: "Developer docs" },
      ],
    },
    {
      label: "Governance",
      links: [
        { href: "internal/governance/adr/index.html", text: "ADRs" },
        { href: "internal/governance/rfc/index.html", text: "RFCs" },
        { href: "internal/governance/backlog/index.html", text: "Backlog" },
  ...
  tech.textContent = "Static HTML · services/frontend/portal/ · No build step";
...
function initLevel3Breadcrumbs() {
  ...
  const isReadme = lastName === "index.html";
  ...
  dirSegs.forEach((seg, i) => {
    ...
    const segPath = segments.slice(0, i + 1).join("/") + "/index.html";
    crumbs.push({
      label: segLabel(seg),
